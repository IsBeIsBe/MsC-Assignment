	...
	public void getGameStats() {
		...
		if (connection != null) {
			...
			try {
				...
				// 1 is the column index here

				// Second Query
				String queryAIWinner = "SELECT COUNT(*) overallGameWinner FROM game WHERE 'overallGameWinner' <> 'Human'";
				ResultSet rsAIWinner = statement.executeQuery(queryGamesPlayed);

				while (rsAIWinner.next()) {
					this.compWins = rsAIWinner.getInt(1);
//					this.humanWins = gamesPlayed - compWins;
				}

				// Third Query
//				********************************************CHECK ABOVE WITH DATABASE
//				***************************************************************************
//				***************************************************************************
